...
int main() {
    for (int j = 0; j < 10; j++) {
        ...
        {
            arr1 = _mm256_loadu_ps(&floatArray[i]);
            arr2 = _mm256_loadu_ps(&floatArray[i + 1]);
            compress = _mm256_sub_ps(arr2, arr1);//从arr2减去arr1
            _mm256_storeu_ps(&comp[i + 1], compress);
        }
        for (; i < length - 1; i++)
        {
        ...
        /*std::ofstream f("D:/BaiduNetdiskDownload/data/ExpIndex_Query/compress_SIMD.txt", std::ios::app);
        if (!f.is_open()) {
            std::cerr << "无法打开文件" << std::endl;
            return 0;
        }
